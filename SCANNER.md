...
A page-wide “message sent” toast is not sufficient proof. OfferUp can confirm only an exact outgoing bubble with Delivered/Read status in a conversation whose item-details page resolves to the approved canonical listing URL. This structure was checked read-only on a delivered conversation; a new complete Apple Events send remains unverified. Missing/ambiguous evidence and Facebook sends record delivery_unconfirmed. In Message delivery status, use Record verified delivery after checking the exact outgoing message in the correct seller conversation. Save the reviewer and evidence. A confirmed send cannot be replayed; a verified failure clears the old approval and requires fresh review. If uncertain, leave it on hold. Tests simulate process contention, interrupted clicks, corruption and reconciliation without sending live messages.
