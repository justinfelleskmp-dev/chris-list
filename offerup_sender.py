...
def send(job, update):
    try:
        with ChromeTab() as page:
            ...
            receipt=check(page,job)
            if receipt:return update(job['id'],'sent','Exact approved message has '+receipt['status']+' in the matching OfferUp conversation',receipt=receipt)
