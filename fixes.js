const sectionStyle=document.createElement('link');sectionStyle.rel='stylesheet';sectionStyle.href='sections.css?v=20260907-sources';document.head.append(sectionStyle);
...
render=function(){
 ...
 const rows=visibleListings();document.querySelectorAll('.cards .card').forEach((card,i)=>{let x=rows[i],label=document.createElement('label'),box=document.createElement('input');box.type='checkbox';box.style.width='auto';box.checked=selected.has(x.id);box.onchange=()=>box.checked?selected.add(x.id):selected.delete(x.id);label.append(box,document.createTextNode(' Select · '+x.platform));card.prepend(label);});
 ...
 if(!primaryGrid.children.length)primaryGrid.innerHTML='<p>No primary listings match these sources.</p>';
 if(!secondary.querySelector('.cards').children.length)secondary.querySelector('.cards').innerHTML='<p>No lower-priority listings match these sources.</p>';
